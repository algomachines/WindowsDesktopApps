...
namespace WorkbenchLib
{
	...
	{
		unsigned char *b = (unsigned char *)&buffer[0];
		...
		int delta = len_of_new_item - len_of_existing_item;
		buffer.resize(buffer_len + delta);

		b = (unsigned char *)&buffer[0];

		for (size_t i = buffer.size() - 1; i >= offset + len_of_new_item; i--)
		{
			b[i] = b[i - delta];
		}
		MemMove(b + offset, new_item, len_of_new_item);
	...
	inline BOOL IsValidASCIIChar(const char &c, bool include_cr_lf=false)
	{
		if (c >= 'a' && c <= 'z') return TRUE;
		if (c >= 'A' && c <= 'Z') return TRUE;
		if (c >= '0' && c <= '9') return TRUE;
	...
	// Remove cr and/or lf at the end of the line
	inline void remove_eol(char *s)
	{
		int len = StrLen(s);
	...
	inline BOOL IsHexChar(char c, int *v=0)
	{
		if (c >= '0' && c <= '9')
		{
			if (v)
	...
	public:
		...
		inline char* GetStringMutable(void)
		{
			return m_s;
		}
		...
		{
			if (IsEqualTo(snew, m_s, TRUE)) return *this;
			...
			if (StrLen(m_s) == len)
				MemMove(m_s, (const char *)snew, len + 1);
		...
		inline BOOL operator < (const char *s0) const 
		{
			return IsLessThan(m_s, s0, TRUE);
		}

		inline BOOL operator > (const char *s0) const 
		{
			return IsGreaterThan(m_s, s0, TRUE);
		}
		...
		inline BOOL operator < 	(STRING &s0) const
		{
			return IsLessThan(m_s, s0, TRUE);
		}
		...
		bool InsertChar(int i, char c);
		bool resize(int sz);

		// c - if not zero, must be allocated with size of at least width - returns the characters removed
		inline BOOL Remove(int i, int width, char *c=0)
		{
			...
			if (c) // c must be allocated with size of at least width
			{
		...
		int GetArrayOfDelimitedItemsEx(STRING *&s_array, char delimiter) const;
		static int GetArrayOfDelimitedItemsEx(const char *s, STRING *&s_array, char delimiter);
		...
		inline bool Right(int n, STRING *&sright) const
		{
			if (n < 0)
				return false;
	...
	inline BOOL STRING::StripDriveLetter(void)
	{
		...
		if ((m_s[0] >= 'a' && m_s[0] <= 'z') || (m_s[0] >= 'A' && m_s[0] <= 'Z'))
		{
			if (m_s[1] == ':')
				return Remove(0, 2);
		}
	...
	inline BOOL STRING::IsRemoteFileSpec(void)
	{
		...
		if (IsEqualTo(m_s, "http://", TRUE))
			return TRUE;

		if (IsEqualTo(m_s, "ftp://", TRUE))
			return TRUE;
