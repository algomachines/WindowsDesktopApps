...
namespace WorkbenchLib
{
	...
	BOOL STRING::IncludesTag(const char *tag, STRING *value/*=0*/, int *istart/*=0*/, int *iend/*=0*/) const
	{
		...
		const char *start = StrStr(m_s, s_tag);
		...
		if (value)
			value->Assign(start + len + 2, (int)(end - start - (len + 2)));

		if (istart)
			*istart = (int)(start + len + 2 - m_s);
	...
	BOOL STRING::ConvertToAbnormalHtml(void)
	{
		...
		for (int i = 0; i<len; i++)
		{
			...
			s1[j++] = '%';
			sprintf_s(&s1[j], (3 * len + 1) - j, "%02X", (WORD)m_s[i]);
			j += 2;
		}
	...
	long STRING::WordCount()
	{
		...
		while (1)
		{
			while (m_s[i] && IsWhiteSpaceChar(m_s[i])) i++;
			if (!m_s[i]) break;
			N++;

			while (m_s[i] && IsWhiteSpaceChar(m_s[i]) == FALSE) i++;
			if (!m_s[i]) break;
		}
	...
	BOOL STRING::NormalizeHtml(void)
	{
		...
		for (int i = 0; i<n - 2; i++)
		{
			...
			{
				BYTE b;
				if (get_BYTE_hex(&m_s[i + 1], b) == TRUE)
				{
					m_s[i] = (char)b;
					MemRemove(&m_s[i + 1], StrLen(&m_s[i + 1]) + 1, 2, 0);
				}
			}
		}
	...
	BOOL STRING::ForceSuffix(const char *suffix, BOOL case_sensitive/*=FALSE*/)
	{
		...
		if (len >= suffix_len)
		{
			if (IsEqualTo(&m_s[len - suffix_len], suffix, case_sensitive) == TRUE)
				return TRUE;
		}
	...
	BOOL STRING::Append_s(const char *s0, const char *s1/*=0*/, const char *s2/*=0*/, const char *s3/*=0*/, const char *s4/*=0*/, const char *s5/*=0*/, const char *s6/*=0*/, const char *s7/*=0*/, const char *s8/*=0*/, const char *s9/*=0*/)
	{
		...
		while (1)
		{
			if (s0) { n = StrLen(s0); MemMove(tmp + i, s0, n); i += n; }
			else break;
			if (s1) { n = StrLen(s1); MemMove(tmp + i, s1, n); i += n; }
			else break;
			if (s2) { n = StrLen(s2); MemMove(tmp + i, s2, n); i += n; }
			else break;
			if (s3) { n = StrLen(s3); MemMove(tmp + i, s3, n); i += n; }
			else break;
			if (s4) { n = StrLen(s4); MemMove(tmp + i, s4, n); i += n; }
			else break;
			if (s5) { n = StrLen(s5); MemMove(tmp + i, s5, n); i += n; }
			else break;
			if (s6) { n = StrLen(s6); MemMove(tmp + i, s6, n); i += n; }
			else break;
			if (s7) { n = StrLen(s7); MemMove(tmp + i, s7, n); i += n; }
			else break;
			if (s8) { n = StrLen(s8); MemMove(tmp + i, s8, n); i += n; }
			else break;
			if (s9) { n = StrLen(s9); MemMove(tmp + i, s9, n); i += n; }
			else break;
			break;
		}
	...
	BOOL STRING::operator >= (const char *s0) const 
	{
		if (strcmp(m_s, s0) >= 0) return TRUE;
		return FALSE;
	}
	...
	BOOL STRING::operator >= (STRING &s0) const 
	{
	...
	{
		if (STRING::GetLength() < len) return;
		m_s[len] = 0;
	}
	...
	{
		trim(m_s, leading, extra, trailing);
	}
	...
	int STRING::GetNextSubStr(int i, STRING &sub_str)
	{
		...
		while (IsWhiteSpaceChar(m_s[i])) i++;
		if (m_s[i] == 0)
			return -1;
	...
	int STRING::NextWord(int i)
	{
		...
		while (1)
		{
			...
			if (i)
			{
				if (IsWhiteSpaceChar(m_s[i - 1]) && !IsWhiteSpaceChar(m_s[i]))
					return (i);
			}
	...
	BOOL STRING::SetFilePath(const char *path)
	{
		...
		STRING tmp;
		if (tmp.LoadFileName(m_s, FALSE, TRUE, TRUE) != TRUE)
			return FALSE;
	...
	BOOL STRING::AppendFILETIME(FILETIME &ft, BOOL bHumanReadable/*=FALSE*/)
	{
		if (bHumanReadable)
		{
			...
			char s0[32];
			sprintf_s(s0, sizeof(s0), "%04d/%02d/%02d %02d:%02d:%02d", st.wYear, st.wMonth, st.wDay, st.wHour, st.wMinute, st.wSecond);
			*this += s0;
			return TRUE;
		}
	...
	BOOL STRING::RemoveSubDir(STRING *removed_name/*=0*/)
	{
		...
		for (int i = n - 2; i >= 0; i--)
		{
			...
			{
				if (removed_name)
					*removed_name = (const char *)(&m_s[i + 1]);
	...
	BOOL STRING::MakeValidFileName(void)
	{
		...
		for (int i = 0; i<n; i++)
		{
			if (IsValidFileNameChar(m_s[i]) == FALSE)
				m_s[i] = '_';
		}
	...
	BOOL STRING::IsValidFileName(void)
	{
		int n = StrLen(m_s);
	...
	{
		unsigned short n;
		if (sscanf_s(m_s, "%hu", &n) != 1) return (0);
		return (n);
	}
	...
	{
		unsigned long n;
		if (sscanf_s(m_s, "%ul", &n) != 1) return (0);
		return n;
	}
	...
	STRING &STRING::operator = (int i)
	{
		...
		sprintf_s(snum, sizeof(snum), "%ld", i);
	...
	BOOL STRING::AppendMax(const char *s0, int n_max)
	{
		...
		m_s = new char[len0 + len1 + 1];

		ASSERT(m_s);

		MemMove(m_s, s_old, len0);
		MemMove(m_s + len0, s0, len1);
	...
	BOOL STRING::Overwrite(int i, const char *c, int width)
	{
		...
		if (org_len < i + width)
		{
			...
			else
			{
				...
				// Make sure that m_s[] is null terminated
				m_s[alloc_size - 1] = 0;
	...
	bool STRING::InsertChar(int ipos, char c)
	{
		if (ipos < 0) 
			return false;
	...
	BOOL STRING::ReplaceAfterLast(const char *s_find, const char *replace_with)
	{
		...
		while (i >= 0)
		{
			if (IsEqualTo(&m_s[i], s_find, TRUE, len_find) == TRUE)
			{
	...
	int STRING::Replace(const char *target, const char *replace_with, int max_num/*=0x7FFF*/)
	{
		...
		while (1)
		{
			const char *c = StrStr(&m_s[i], target, TRUE);
			if (!c)
				return n;
		...
		)
	{
		...
		if (sign != '-' && (short)strlen(s_tmp) <= max_output_str_len)
		{
		...
		if (sign == '-' && (short)strlen(s_tmp) < max_output_str_len)
		{
			output_var_str[0] = sign;
			memmove(&output_var_str[1], s_tmp, strlen(s_tmp) + 1);
			return;
		}
...
#include "intrin.h"
	...
	BOOL STRING::InsertAtDelimitedPosition(const char *v, int zero_based_idx, char delim_char /* = '/t' */)
	{
		...
		c1 = StrStr(c0, sdelim);
		if (!c1)
			c1 = this->m_s + strlen(this->m_s);

		STRING *end_part = Cut(c0 - this->m_s);
		*this += v;
	...
	bool STRING::AssignDelimitedItem(const char *m_s, int item, char delimiter)
	{
		...
		while (m_s[i])
		{
			...
			if (m_s[i] == '\"') // starting quote found
			{
				i++;
				istart = i;
				while (m_s[i] && m_s[i] != '\"') i++; // find ending quote
				if (m_s[i] == 0) break; // no matching quote found
				iend = i - 1;
				...
				if (nitems == item)
				{
					this->Insert(0, &m_s[istart], iend - istart + 1);
					return true;
				}
				nitems++;
				i++;
	...
	int STRING::GetArrayOfDelimitedItemsEx(STRING *&s_array, char delimiter) const
	{
		return GetArrayOfDelimitedItemsEx(m_s, s_array, delimiter);
	}

    int STRING::GetArrayOfDelimitedItemsEx(const char *s, STRING *&s_array, char delimiter)
	{
		if (delimiter == ' ')
			return 0;
	...
	int STRING::GetArrayOfDelimitedItems(STRING *&array, char delimiter)
	{
		...
		const char *c0 = m_s;
		const char *c1 = strchr(m_s, delimiter);
	...
	bool STRING::SwapInSingleWildcardCharactersFromNamingConvention(const char *naming_convention, STRING *s_swapped/*=0*/)
	{
		...
		int idx_number;
		BOOL status = matches_naming_convention(m_s, naming_convention, idx_number, 0, 0, 1, idx_scw, 20);
		if (status == FALSE)
			return false;
	...
	BOOL STRING::MatchesNamingConvention(const char *naming_convention, const char *sample_name/*=0*/, BOOL exclude_path/*=TRUE*/)
	{
		int number_idx;
		int number_n;
		BOOL status = matches_naming_convention(m_s, naming_convention, number_idx, &number_n, sample_name, exclude_path);
	...
	BOOL STRING::RemoveIndexFromName(const char *naming_convention, BOOL exclude_path/*=TRUE*/)
	{
		...
		const char *c = m_s;
		if (number_idx && m_s[number_idx - 1] == '.' || m_s[number_idx - 1] == '_')
		{
			number_n++;
			number_idx--;
		}
	...
	BOOL STRING::GetIndexUsingNamingConvention(const char *naming_convention, long &index, BOOL exclude_path/*=TRUE*/)
	{
		...
		int number_idx;
		if (matches_naming_convention(m_s, naming_convention, number_idx, 0, 0, exclude_path) == FALSE)
			return FALSE;
		...
		int n = 0;
		while (m_s[number_idx + n] >= '0' && m_s[number_idx + n] <= '9')
			n++;
	...
	{
		STRING tmp;
		if (tmp.LoadFileName(m_s, TRUE, TRUE, FALSE) == FALSE)
			return FALSE;
...
#define MAX_DOT  32
		...
		{
			index_of_number_src = StrLen(m_s) - (StrLen(naming_convention_src) - i_num_src);
		}
		...
		for (isrc = 0; isrc<i_num_src; isrc++)
		{
			...
			if (naming_convention_src[isrc] == '*')
			{
				...
				{
					while (m_s[i] != naming_convention_src[isrc + 1])
					{
						ASSERT(j<new_alloc);
						tmp[j++] = m_s[i++];
						continue;
			...
			else
			{
				...
				ASSERT(m_s[i] == naming_convention_src[isrc]);
				ASSERT(j<new_alloc);
				tmp[j++] = m_s[i++];
			}
		}
